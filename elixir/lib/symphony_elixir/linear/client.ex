defmodule SymphonyElixir.Linear.Client do
    ...
    cond do
      ...
      true ->
        with {:ok, assignee_filter} <- routing_assignee_filter() do
          do_fetch_by_states(scope, Config.linear_active_states(), assignee_filter)
        end
    end
  end
  ...
  @doc false
  @spec scope_query_for_test({:project_slug, String.t()} | {:team_key, String.t()}, [String.t()], String.t() | nil) ::
          {String.t(), map()}
  def scope_query_for_test(scope, state_names, after_cursor \\ nil) do
    build_scope_query(scope, state_names, after_cursor)
  end
  ...
  defp do_fetch_by_states_page(scope, state_names, assignee_filter, after_cursor, acc_issues) do
    {query, variables} = build_scope_query(scope, state_names, after_cursor)
  ...
  defp build_scope_query({:project_slug, project_slug}, state_names, after_cursor) do
    {@query,
     %{
  ...
  defp build_scope_query({:team_key, team_key}, state_names, after_cursor) do
    {@team_query,
     %{
       teamKey: team_key,
       stateNames: state_names,
